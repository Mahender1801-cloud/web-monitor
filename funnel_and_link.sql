...
begin
  with sess as (
    select distinct session_id from public.rum_events
    where created_at >= p_from and created_at <= p_to and session_id is not null
  ),
  prod as (
    select distinct session_id from public.rum_events
    where created_at >= p_from and created_at <= p_to and path like '/products%'
  ),
  cartv as (
    select distinct session_id from public.rum_events
    where created_at >= p_from and created_at <= p_to and path like '/cart%'
  ),
  atc as (
    select distinct session_id from public.funnel_events
    where created_at >= p_from and created_at <= p_to and event_type = 'add_to_cart'
  ),
  chk as (
    select distinct session_id from public.funnel_events
    where created_at >= p_from and created_at <= p_to and event_type = 'checkout_click'
  ),
  purch as (
    select distinct rum_session as session_id from public.shop_orders
    where created_at >= p_from and created_at <= p_to and rum_session is not null
  )
  select json_build_object(
    'sessions',       (select count(*) from sess),
    'viewed_product', (select count(*) from prod),
    'added_to_cart',  (select count(*) from atc),
    'viewed_cart',    (select count(*) from cartv),
    'checkout_click', (select count(*) from chk),
    'purchased',      (select count(*) from purch)
  ) into result;
  return result;
end;
$$;
