...
async function timedFetch(url, opts={}){
  const t0 = Date.now();
  const r = await fetch(url, { redirect:'follow', headers:{'User-Agent':UA,'Accept':'text/html,*/*'}, ...opts });
...
async function analyze(url){
  const main = await timedFetch(url);
  ...
  // ---- sub-scores ----------------------------------------------------------
  const perf = clamp(
    0.34*grade(main.ttfb, 200, 1200) +          // server response
    0.24*grade(main.total, 400, 3000) +         // full HTML delivery
    0.18*grade(weight/1024, 500, 4000) +        // total sampled weight (KB)
    0.16*grade(renderBlocking, 1, 12) +         // render-blocking resources
    0.08*grade(thirdParty, 2, 20)               // third-party scripts
  );
  const seo = clamp(
  ...
  return {
    ...
    metrics: {
      ttfb_ms: main.ttfb, load_ms: main.total,
